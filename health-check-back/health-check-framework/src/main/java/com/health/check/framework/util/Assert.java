...
public class Assert {
    ...
    public static <T> T notNull(T object, String errorMsgTemplate, Object... params) throws BusinessException {
        return notNull(object, () -> new BusinessException(StrUtil.format(errorMsgTemplate, params)));
    }
     ...
     * @param size  长度
     * @return 检查后的下标
     * @throws BusinessException         如果size &lt; 0 抛出此异常
    ...
    public static int checkBetween(int value, int min, int max) {
        if (value < min || value > max) {
            throw new BusinessException(StrUtil.format("Length must be between {} and {}.", min, max));
        }
        return value;
    }
